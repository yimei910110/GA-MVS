...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, 
        ...
        with_gt=True, max_h=None, max_w=None, base_image_size=None,
    ...
    def canny_detect(self,img_path):

        img = cv2.imread(img_path)
        return cv2.Canny(img, 64, 128)
        #return cv2.Canny(img, 128, 200)
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if self.max_h is not None and self.max_w is not None:
                if self.max_hw_mode == "scale":
                    ...
                    intrinsics = self.crop_cam(intrinsics, h=old_h, w=old_w, new_h=self.max_h, new_w=self.max_w,
                                               base=self.base_image_size)
            if self.base_image_size is not None:
                old_h = img.shape[0]
                old_w = img.shape[1]
                img = self.crop_img(img, new_h=None, new_w=None, base=self.base_image_size)
                intrinsics = self.crop_cam(intrinsics, h=old_h, w=old_w, new_h=None, new_w=None,
                                           base=self.base_image_size)
            ...
            if i == 0:  # reference view
                canny_img = self.canny_detect(img_filename)
                # print('debug in dtu_yao_eval canny_img', canny_img.shape) # (1200, 1600)
                canny_img = cv2.resize(canny_img, [new_w, new_h], interpolation=self.img_interp)
                # print('debug in dtu_yao_eval after resize', canny_img.shape) # (1152, 1600)
                ref_canny_imgs.append(canny_img)
        ...
        ref_canny_imgs = np.stack(ref_canny_imgs, axis=0)

        sample_interval = depth_range / self.depth_num
        sample_depth = []

        for i in range(self.depth_num):
            sample_depth.append(np.ones([ref_imgs[str(0)].shape[0], ref_imgs[str(0)].shape[1]], dtype=np.float32) * (
